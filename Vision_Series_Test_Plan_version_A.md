...
This section covers the functional testing of post-assembly boards prior to stack integration. Each PCB is populated with pre-tested IC components from Stage 1. Testing verifies that the soldering and board-level interconnects are reliable and that the individual sub-systems function correctly.

#### 6.2.1 MAG-VIS4000x-N (Imaging PCB)

##### Test flow

The production test flow for the Imaging PCB is depicted below. Pre-tested image sensor components from Stage 1 are assembled onto the PCB, followed by functional verification of the board-level system.

![MAG-VIS4000x-N Production Test Flow](vision-module-testing-diagrams/VIS4000x_production_test_flow.png)
*Figure 6: MAG-VIS4000x-N Production Test Flow*

*Production test stages:*
- **VIS (Vision Test):** Functional test of the assembled imaging PCB. Validates that the board-level interconnects and the image sensor readout chain operate correctly after PCB population.

##### Test procedures
- **Functional Test:** Validates the assembled PCB functionality, including power delivery, signal integrity, and image sensor communication.

#### 6.2.2 MAG-VIS4003x-N (Serializer PCB)

##### Test flow

The production test flow for the Serializer PCB is depicted below. Pre-tested serializer and DC/DC converter components from Stage 1 are assembled onto the PCB, followed by functional verification.

![MAG-VIS4003x-N Production Test Flow](vision-module-testing-diagrams/VIS4003x_production_test_flow.png)
*Figure 7: MAG-VIS4003x-N Production Test Flow*

*Production test stages:*
- **VIS (Vision Test):** Functional test of the assembled serializer PCB. Validates the CXP link functionality and power delivery sub-system at the board level.

##### Test procedures
- **Functional Test:** Validates the assembled serializer PCB functionality, including CXP link and power delivery sub-system.

#### 6.2.3 MAG-VIS4001x-N (Power PCB)

##### Test flow

The production test flow for the Power PCB is depicted below. Pre-tested DC/DC converter components from Stage 1 are assembled onto the board, followed by connectivity and functional testing to verify voltage regulation and ripple performance.

![MAG-VIS4001x-N Production Test Flow](vision-module-testing-diagrams/VIS4001x_production_test_flow.png)
*Figure 8: MAG-VIS4001x-N Production Test Flow*

*Production test stages:*
- **CONN (Connectivity Test):** Checks for shorts on the DUT interface at the board level, verifying that PCB assembly and soldering have not introduced connection faults.
- **VIS (Vision Sub-board Test):** Verifies functionality of the DCDC sub-board on the MAG-VIS400xx PCBs, including voltage-level accuracy and ripple under the expected load-profile.
